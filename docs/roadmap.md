...
## ~~v1.4.0 — Docker & CI/CD Integration~~ ✅ Released

- `VAULTIC_AGE_KEY` environment variable: use a private key from env instead of file — essential for CI/CD
- `--stdout` flag on `decrypt` and `resolve`: raw output to stdout for piping (no UI messages)
- `vaultic ci export`: export secrets in CI-specific formats (`--format github|gitlab|generic`, `--mask` for GitHub Actions)
- `.dockerignore` safety check in `vaultic status`: warns when Docker files are present but `.env` is not in `.dockerignore`
- `vaultic validate` now exits with code 2 for validation failures (distinguishable from other errors in CI)
